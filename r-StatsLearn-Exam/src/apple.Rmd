...
# bigger depth
ada.depth <- rpart.control(maxdepth=4, cp=-1, maxcompete=1, xval=0)
...
my.gbm <- create.Learner("SL.gbm", name_prefix="GBM", detailed_names=T,
                         ...
                         tune=list(interaction.depth=c(1,3)))
...
Si capisce che mettere strong learners eccessivamente complessi, porta l'algoritmo ad assegnare coefficiente 0 a più modelli. Si capisce quindi la logica dietro AdaBoost dei weak learners.
...
```{r}
# sl.full.cv.summ <- summary(sl.full.cv)

# sl.full.cv.summ$Table$CIinf <- sl.full.cv.summ$Table$Ave - qnorm(0.975)*sl.full.cv.summ$Table$se
# sl.full.cv.summ$Table$CIsup <- sl.full.cv.summ$Table$Ave + qnorm(0.975)*sl.full.cv.summ$Table$se
# sl.full.cv.summ$Table
# 
# dotchart(sort(sl.full.cv.summ$Table[,2], decreasing=TRUE),
#          labels=sl.full.cv.summ$Table[,1][order(sl.full.cv.summ$Table[,2], decreasing=TRUE)],
#          main="Learners 5-fold CV error", pch=19, xlab="CV error", pt.cex=1.5)
# arrows(
#   x0 = sl.full.cv.summ$Table$CIinf[order(sl.full.cv.summ$Table[,2], decreasing=TRUE)], 
#   y0 = 1:dim(sl.full.cv.summ$Table)[1], 
#   x1 = sl.full.cv.summ$Table$CIsup[order(sl.full.cv.summ$Table[,2], decreasing=TRUE)], 
#   y1 = 1:dim(sl.full.cv.summ$Table)[1], 
#   code = 3, 
#   angle = 90, 
#   length = 0.05
# )
