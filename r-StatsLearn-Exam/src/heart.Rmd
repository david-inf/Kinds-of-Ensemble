...
```{r}
## rivedere e rifare che sto pacchetto m'ha fottuto
ada.stump <- rpart.control(maxdepth=30, cp=-1, minsplit=20, xval=10)
ada.depth2 <- rpart.control(maxdepth=2, cp=-1, minsplit=20, xval=10)
...
```{r}
set.seed(111)
adaboost.2 <- ada::ada(x=as.matrix(dat.train[,-target]),
                     y=dat.train[,target],
                     test.x=as.matrix(dat.test[,-target]),
                     test.y=dat.test[,target],
                     loss="exponential", type="discrete",
...
```{r}
paste0("Boosting trees (stumps): ", M.cross)
paste0("Boosting trees (depth2): ", M.cross)
```
